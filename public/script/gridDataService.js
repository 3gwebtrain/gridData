...
.service('apiService', ['$rootScope', "jsonMaker",  "$q",  function( $rootScope, jsonMaker, $q  ){
	...
	this.generateData = function( size ) {
		...
		while( baseSize < size ) {
			baseSize++;
			newData.push( {
				"mk" : jsonMaker.createDigit( 3 ),
				"controlNumber": jsonMaker.createDigit( 3 ),
				"control": jsonMaker.createPrefixedDigit( ),
				"prefix":jsonMaker.createAlphaNumaric( ),
				"base": jsonMaker.createAlphaValue( 3 ),
				"suffix":jsonMaker.createAlphaSuffix( 5 ),
				"sp":jsonMaker.createDigit(  2 ),
				"trimin":jsonMaker.createDigit( 6 ),
				"offline":jsonMaker.createDigit( 6 ),
				"engine":jsonMaker.createAlphaSuffix( 5 ),
				"zip":jsonMaker.createDigit( 6 ),
				"station":  jsonMaker.createAlphaValue( 3 ),
